...
async def warn_user(update: Update, context: ContextTypes.DEFAULT_TYPE, user_id: int, username: str):
    ...
    if count == 1:
        await mute_user(update, context, user_id, 3600)
        await update.message.reply_text(f"تحذير 1 لـ @{username}: كتم لمدة ساعة.")
    elif count == 2:
        await mute_user(update, context, user_id, 86400)
        await update.message.reply_text(f"تحذير 2 لـ @{username}: كتم لمدة 24 ساعة.")
    elif count == 3:
        await mute_user(update, context, user_id, 3*86400)
        await update.message.reply_text(f"تحذير 3 لـ @{username}: كتم لمدة 3 أيام.")
    else:
        await context.bot.ban_chat_member(chat_id=update.effective_chat.id, user_id=user_id)
        await update.message.reply_text(f"تم حظر @{username} نهائياً بسبب تكرار الكلمات السيئة.")
...
async def welcome_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    for member in update.message.new_chat_members:
        ...
        welcome_text = f"أهلاً وسهلاً بـ {member.first_name}!\n\n" \
                       ...
                       "3. أي مخالفة راح تتعرض للتحذير والكتم أو الحظر.\n\n" \
